#!/usr/bin/env bash
#
# galaxy-statusline update script
# https://github.com/kellyredding/galaxy
#
# This script is fetched and executed by `galaxy-statusline update`.
...
Update script: https://raw.githubusercontent.com/${REPO}/main/tools/statusline/scripts/update.sh
EOF
}
...
main() {
  ...
  local script_url="https://raw.githubusercontent.com/${REPO}/main/tools/statusline/scripts/update.sh"
  ...
  if [[ "$PREVIEW" == true ]]; then
    ...
    if [[ "$current_version" == "$latest_version" ]]; then
      ...
      echo "    galaxy-statusline update force"
    else
  ...
  echo -e "${CYAN}${action_word} galaxy-statusline${NC}"
  echo ""
  echo "  Current version:  ${current_version}"
  ...
  # Binary is named galaxy-statusline-{version}-{platform} in the tarball
  ...
  echo ""
  echo "  Verify: galaxy-statusline version"
}
