...
describe GalaxyStatusline::Config do
  describe ".default" do
    it "creates a config with default values" do
      config = GalaxyStatusline::Config.default

      config.version.should eq(GalaxyStatusline::VERSION)
  ...
  describe ".load" do
    it "creates default config when file doesn't exist" do
      ...
      FileUtils.rm_rf(GalaxyStatusline::CONFIG_DIR.to_s)

      config = GalaxyStatusline::Config.load
      config.branch_style.should eq("symbolic")

      # Should have created the file
      File.exists?(GalaxyStatusline::CONFIG_FILE).should eq(true)
    end
  end
  ...
  describe "#to_pretty_json" do
    it "produces valid JSON" do
      ...
      parsed = GalaxyStatusline::Config.from_json(json)
