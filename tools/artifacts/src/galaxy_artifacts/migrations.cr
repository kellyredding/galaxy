...
module GalaxyArtifacts
  module Migrations
    ...
    def self.migrate_database(db : DB::Database)
      ...
      current_version = GalaxyArtifacts::VERSION
      ...
      if version_greater_than?(stored_version, current_version)
        STDERR.puts "[galaxy-artifacts] Warning: Database schema version (#{stored_version}) is newer than CLI version (#{current_version})"
        return
      end
