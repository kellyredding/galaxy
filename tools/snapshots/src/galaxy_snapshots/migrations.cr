...
module GalaxySnapshots
  module Migrations
    ...
    def self.migrate_database(db : DB::Database)
      ...
      current_version = GalaxySnapshots::VERSION
      ...
      if version_greater_than?(stored_version, current_version)
        STDERR.puts "[galaxy-snapshots] Warning: Database schema version (#{stored_version}) is newer than CLI version (#{current_version})"
        return
      end
