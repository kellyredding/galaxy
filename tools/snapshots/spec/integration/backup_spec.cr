...
describe "CLI backup command", tags: "integration" do
  ...
  describe "create and prune" do
    it "creates a backup when backups are enabled" do
      backup_dir = Path.new(Dir.tempdir) / "galaxy-snapshots-backup-cli-#{Random.rand(100000)}"
      begin
        # Enable backups with custom path via shared Galaxy config
        shared_config = {
        ...
        GalaxySnapshots::Database.save_snapshot(1_i64, "Backup CLI", "content")
        flush_wal
  ...
  describe "--prune-only" do
    it "prunes old backup directories" do
      backup_dir = Path.new(Dir.tempdir) / "galaxy-snapshots-prune-cli-#{Random.rand(100000)}"
      begin
        ...
        # Configure custom backup path via shared Galaxy config
        shared_config = {
          "_schema_version" => "0.0.1",
          "backups"         => {
            "enabled"        => true,
    ...
    it "shows nothing to prune message when empty" do
      backup_dir = Path.new(Dir.tempdir) / "galaxy-snapshots-prune-empty-cli-#{Random.rand(100000)}"
      begin
        Dir.mkdir_p(backup_dir)
  ...
  describe "help" do
    it "shows backup help referencing galaxy config" do
