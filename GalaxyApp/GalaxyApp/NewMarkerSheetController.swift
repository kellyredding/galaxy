...
/// Presents the new-marker dialog as a standalone modal window.
/// Mirrors the NewSessionSheetController pattern — a regular
/// NSWindow hosting a SwiftUI view, rather than `beginSheet`,
/// so the NSHostingView renders with the same default background
/// as Settings and New Session.
class NewMarkerSheetController: NSObject, NSWindowDelegate {
    private static var active: NewMarkerSheetController?

    private let window: NSWindow
    private var themeObserver: AnyCancellable?

    /// Show the new-marker dialog as an app-modal window.
    /// No-ops if already open (prevents double-open).
    static func present(on parentWindow: NSWindow) {
        guard active == nil else { return }

        let controller = NewMarkerSheetController(parentWindow: parentWindow)
        active = controller
        controller.show()
    ...
    private init(parentWindow: NSWindow) {
        ...
        let view = NewMarkerView { [weak self] in
            self?.dismiss()
        }
